...
### Requirement: Activation du context caching sur le transcript
...
### Requirement: Réponse en JSON structuré
Le module SHALL demander à DeepSeek de retourner un JSON avec les champs `summary` (string) et `key_points` (liste d'objets `{text, timestamp_seconds, timestamp_label}`), et SHALL parser ce JSON pour construire l'`AnalysisResult`.

#### Scenario: JSON valide retourné par DeepSeek
- **WHEN** DeepSeek retourne une réponse JSON conforme au schéma attendu
- **THEN** `analyse_transcript()` parse le JSON et retourne un `AnalysisResult` valide

#### Scenario: JSON invalide retourné par DeepSeek
- **WHEN** DeepSeek retourne une réponse non parseable comme JSON valide
